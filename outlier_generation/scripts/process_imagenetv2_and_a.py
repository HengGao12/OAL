...
for name in f1:
    ...
    shutil.copytree(os.path.join(
        '/nobackup-slow/dataset/imagenetv2/imagenetv2-matched-frequency-format-val/', str(index)),
        os.path.join(
            '/nobackup-slow/dataset/imagenetv2/processed/', copy_name)
    )
    copy_name += 'a'

# for imageneta
name_list = os.listdir('/nobackup-slow/dataset/imageneta/imagenet-a/')
index = 0
for name in f1:
    # index = name_list.index(name)
    if name in name_list:
        shutil.copytree(os.path.join(
            '/nobackup-slow/dataset/imageneta/imagenet-a', name),
            os.path.join(
                '/nobackup-slow/dataset/imageneta/processed/', str(index))
        )
    index += 1
