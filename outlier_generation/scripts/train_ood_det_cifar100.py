...
parser.add_argument('--epochs', '-e', type=int, default=100, help='Number of epochs to train.')  # original: 200
parser.add_argument('--learning_rate', '-lr', type=float, default=0.005, help='The initial learning rate.')
parser.add_argument('--batch_size', '-b', type=int, default=128, help='Batch size.') # original 256
...
parser.add_argument('--my_info', type=str, default='samples')
...
train_data_in = dset.CIFAR100('../data/dataset/cifarpy', train=True, transform=train_transform, download=True)
test_data = dset.CIFAR100('../data/dataset/cifarpy', train=False, transform=test_transform, download=True)
...
ood_data = dset.ImageFolder(root="../nobackup-fast/txt2img-samples-cifar100-demo/" + args.my_info,
...
if args.r50:
    ...
    # net = ResNet_Model(name='resnet18', num_classes=num_classes)
    net = ResNet18_32x32(num_classes=100).cuda()
    # net = ResNet_Model(name='resnet34', num_classes=num_classes)
# net = ResNet_Model(name='resnet18', num_classes=num_classes)

net.load_state_dict(torch.load('../../results/cifar100_resnet18_32x32_base_e100_lr0.1_default/s0/best.ckpt'), strict=False)
...
with open(os.path.join(args.save, args.dataset + calib_indicator + '_' + args.model + '_s' + str(args.seed) +
                                  '_' + save_info + '_training_results-1-1-1_7_23.csv'), 'w') as f:
...
np.save('./cifar100-dream-ood-test_loss-oe-resnet18-bs-128-oe_bs-256-t_bs-200-e-100-lr-0.005-rs-32-v100-7_23.npy', np.array(losses))  # report 3 runs
...
# # fig.colorbar(i)
# plt.savefig('./DreamOOD-training-dynamics-e100.png', dpi=700)
