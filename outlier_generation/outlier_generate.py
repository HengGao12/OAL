...
def main():
    ...
    parser.add_argument(
        "--loaded_embedding",
        type=str,
        default='/nobackup-slow/dataset/diffusion/outlier_npos_embed.npy'
    )
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default="/nobackup-slow/dataset/diffusion/sd-v1-4.ckpt",
            ...
            with model.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        # uncomment the following code for generating outliers
                        b,c,h,w = samples_ddim.shape  # 3, 4, 64, 64
                        # plot the latent representation before taking averaging
                        # plt.imshow(samples_ddim[0].permute(1, 2, 0).cpu().numpy())
                        # plt.axis('off')
                        # plt.savefig('./ood_latent_representation/{}.png'.format(n), dpi=700, bbox_inches='tight', pad_inches=0)
                        
                        samples_ddim_mean = torch.mean(samples_ddim, dim=1).squeeze(1)  # channel mean
                        # # plt.imshow(np.transpose(samples_ddim[0].cpu().numpy(), (1, 2, 0)))
                        # img = samples_ddim[0].squeeze(0).cpu().numpy()
                        # max_val = img.max()
                        # min_val = img.min()
                        # normalized_img = (img - min_val) / (max_val - min_val)
                        
                        # plt.imsave('./noisy_feature_map.png', np.ascontiguousarray(np.transpose(normalized_img, (1, 2, 0))), dpi=700)
                        
                        deep_ood_samples.append(samples_ddim_mean)
                        ...
                        # plot the corresponding image of the latent representation
                        # plt.imshow(x_checked_image_torch[0].permute(1, 2, 0).cpu().numpy())
                        # plt.axis('off')
                        # plt.savefig('./ood_latent_representation/{}_image.png'.format(n), dpi=700, bbox_inches='tight', pad_inches=0)                        
                ...
                # When you need to obtain deep outliers, please uncomment these codes
                deep_ood_samples = torch.stack(deep_ood_samples, dim=0)
                # print(deep_ood_samples.shape)
                torch.save(deep_ood_samples, './deep_ood_embedding_cifar100_plus-test.pt')  # Here we get the Deep OOD Samples
                toc = time.time()
                print('The time used for generation is:{} s.'.format(toc-tic))
