...
class RandomImages(torch.utils.data.Dataset):
    ...
    def __init__(self, transform=None, exclude_cifar=False):
        ...
        data_file = np.load('/nobackup-slow/dataset/300K_random_images.npy')
...
class SDImages(torch.utils.data.Dataset):
    ...
    def __init__(self, transform=None, exclude_cifar=False):
        ...
        data_file = []
        for item in list(os.listdir('/nobackup-slow/dataset/sd/txt2img-samples/samples')):
            data_file.append(os.path.join('/nobackup-slow/dataset/sd/txt2img-samples/samples', item))
